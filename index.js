...
AuthResource.prototype.initPassport = function (requestedModule) {
    if (this.initialized)
        return;
    ...
    var socialAuthCallback = function (token, tokenSecret, profile,
        ...
        userCollection.store.first({socialAccountId: String(profile.id)}, function (
            ...
            if (user) {
                ...
                if (!user.password)
                    update.password = fakeLogin.password;
                if (!user.username)
    ...
    if (config.allowTwitter) {
        ...
        if (requestedModule.endsWith('-token')) {
            debug('Initializing Twitter Login with token');
            passport.use(new TwitterTokenStrategy(config, socialAuthCallback));
        }
        else {
            debug('Initializing Twitter Login, cb: %s', cbURL);
            config.callbackURL = cbURL;
            passport.use(new TwitterStrategy(config, socialAuthCallback));
        }
    }
    ...
    if (config.allowFacebook) {
        ...
        if (requestedModule.endsWith('-token')) {
            debug('Initializing Facebook Login with token');
            passport.use(new FacebookTokenStrategy(config, socialAuthCallback));
        }
        else {
            debug('Initializing Facebook Login, cb: %s', cbURL);
            config.callbackURL = cbURL;
            passport.use(new FacebookStrategy(config, socialAuthCallback));
        }
    }
    ...
    if (config.allowGitHub) {
        ...
        if (requestedModule.endsWith('-token')) {
            debug('Initializing GitHub Login with token');
            passport.use(new GitHubTokenStrategy(config, socialAuthCallback));
        }
        else {
            debug('Initializing GitHub Login, cb: %s', cbURL);
            config.callbackURL = cbURL;
            passport.use(new GitHubStrategy(config, socialAuthCallback));
        }
    }
    ...
    if (config.allowGoogle) {
        ...
        if (requestedModule.endsWith('-token')) {
            debug('Initializing Google Login with token');
            passport.use(new GoogleTokenStrategy(config, socialAuthCallback));
        }
        else {
            debug('Initializing Gooble Login, cb: %s', cbURL);
            config.callbackURL = cbURL;
            passport.use(new GoogleStrategy(config, socialAuthCallback));
        }
    }
    ...
    if (config.allowWeibo) {
        ...
        if (requestedModule.endsWith('-token')) {
            debug('Initializing Weibo Login with token');
            passport.use(new WeiboTokenStrategy(config, socialAuthCallback));
        }
        else {
            debug('Initializing Weibo Login, cb: %s', cbURL);
            config.callbackURL = cbURL;
            passport.use(new WeiboStrategy(config, socialAuthCallback));
        }
    }
...
AuthResource.prototype.handle = function (ctx, next) {
    ...
    if (ctx.url === '/logout') {
        if (ctx.res.cookies)
            ctx.res.cookies.set('sid', null, {overwrite: true});
    ...
    switch (parts[0]) {
        ...
        case 'twitter':
            if (this.config.allowTwitter) {
                requestedModule = parts[1] === 'token' ? 'twitter-token' : 'twitter';
            }
            break;
        case 'facebook':
            if (this.config.allowFacebook) {
                requestedModule = parts[1] === 'token' ? 'facebook-token' : 'facebook';
        ...
        case 'github':
            if (this.config.allowGitHub) {
                requestedModule = parts[1] === 'token' ? 'github-token' : 'github';
        ...
        case 'google':
            if (this.config.allowGoogle) {
                requestedModule = parts[1] === 'token' ? 'google-token' : 'google';
        ...
        case 'weibo':
            if (this.config.allowWeibo) {
                requestedModule = parts[1] === 'token' ? 'weibo-token' : 'weibo';
            }
            break;
        default:
            break;
        ...
        if (ctx.query.redirectURL && this.config.allowedRedirectURLs) {
            try {
                ...
                if (ctx.query.redirectURL.match(this.regEx)) {
                    ...
                    if (ctx.res.cookies)
                        ctx.res.cookies.set('_passportReturnUrl', ctx.query.redirectURL);
                }
                else {
